...
compute_overlap_metrics <- function(mapA, mapB, idA, idB) {
  ...
  inter <- inter %>%
    mutate(
      ...
      metric_1 = A_intersect_B,
      metric_2 = A_intersect_B / A_union_B,
      metric_3 = A_intersect_B / pmin(A, B),
      metric_4 = A_intersect_B / pmax(A, B),
      metric_5 = A_intersect_B / A_intersect_B_perimeter
    ) %>%
    st_drop_geometry()
...
edges_91_01 <- get_overlap_pairs(upazilas91, upazilas01, "ipum1991", "ipum2001", "u91", "u01", 1e-75)
edges_01_11 <- get_overlap_pairs(upazilas01, upazilas11, "ipum2001", "ipum2011", "u01", "u11", 1e-75)
edges_91_11 <- get_overlap_pairs(upazilas91, upazilas11, "ipum1991", "ipum2011", "u91", "u11", 1e-75)
